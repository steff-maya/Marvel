package com.stef.android.marvel.anim.heplers;
...
public class DotsView extends View {
    ...
    private void init() {
        maxDotSize = UtilsAnimation.dpToPx(getContext(), 4);
    ...
    private void updateInnerDotsPosition() {
        if (currentProgress < 0.3f) {
            this.currentRadius2 = (float) UtilsAnimation.mapValueFromRangeToRange(currentProgress, 0, 0.3f, 0.f, maxInnerDotsRadius);
        } else {
        ...
        if (currentProgress < 0.2) {
            ...
            this.currentDotSize2 = (float) UtilsAnimation.mapValueFromRangeToRange(currentProgress, 0.2f, 0.5f, maxDotSize, 0.3 * maxDotSize);
        } else {
            this.currentDotSize2 = (float) UtilsAnimation.mapValueFromRangeToRange(currentProgress, 0.5f, 1f, maxDotSize * 0.3f, 0);
        }
    ...
    private void updateOuterDotsPosition() {
        if (currentProgress < 0.3f) {
            this.currentRadius1 = (float) UtilsAnimation.mapValueFromRangeToRange(currentProgress, 0.0f, 0.3f, 0, maxOuterDotsRadius * 0.8f);
        } else {
            this.currentRadius1 = (float) UtilsAnimation.mapValueFromRangeToRange(currentProgress, 0.3f, 1f, 0.8f * maxOuterDotsRadius, maxOuterDotsRadius);
        }

        if (currentProgress < 0.7) {
            this.currentDotSize1 = maxDotSize;
        } else {
            this.currentDotSize1 = (float) UtilsAnimation.mapValueFromRangeToRange(currentProgress, 0.7f, 1f, maxDotSize, 0);
        }
    }

    private void updateDotsPaints() {
        if (currentProgress < 0.5f) {
            float progress = (float) UtilsAnimation.mapValueFromRangeToRange(currentProgress, 0f, 0.5f, 0, 1f);
            ...
            float progress = (float) UtilsAnimation.mapValueFromRangeToRange(currentProgress, 0.5f, 1f, 0, 1f);
    ...
    private void updateDotsAlpha() {
        float progress = (float) UtilsAnimation.clamp(currentProgress, 0.6f, 1f);
        int alpha = (int) UtilsAnimation.mapValueFromRangeToRange(progress, 0.6f, 1f, 255, 0);
    ...
    public void setColors(int startColor, int endColor) {
        color1 = startColor;
        color2 = UtilsAnimation.darkenColor(color1, 1.1f);
        color4 = endColor;
        color3 = UtilsAnimation.darkenColor(color4, 1.1f);
    }
}